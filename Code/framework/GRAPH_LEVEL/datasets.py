...
def load_tudata(feature_engineerings, dataset_name='DD', cleaned=False, split_seed=12345, batch_size=32, remove_large_graph=True, folds=10):
    print(f"****{dataset_name}*****")

    if dataset_name == "userDataset":
        dataset = CustomTUDataset(path, dataset_name, cleaned=cleaned)
    else:
...
def load_data(feature_engineerings, dataset_name='DD', cleaned=False, split_seed=12345, batch_size=32, remove_large_graph=True, folds=10):
    if dataset_name in ['mr', 'ohsumed', 'R8', 'R52', 'TREC', 'ag_news', 'WebKB', 'SST1', 'SST2']:
        ...
        for fe_name in feature_engineerings:
            ...
            if transform_class:
                if fe_name == 'SIGN':
                    ...
                    pass
                elif fe_name == 'TargetIndegree':

                    transforms_list.append(transform_class(max_degree = 3))
                else:
                    transforms_list.append(transform_class())
            else:
                print(f"Warning: Transform {fe_name} not found.")
        ...
        return load_tudata(feature_engineerings, dataset_name, cleaned, split_seed, batch_size, remove_large_graph, folds=folds)

class CustomTUDataset(TUDataset):
    def __init__(self, root, name, transform=None, pre_transform=None, pre_filter=None,
                 use_node_attr=False, use_edge_attr=False, cleaned=False):
        super(CustomTUDataset, self).__init__(root, name, transform, pre_transform, pre_filter,
                                              use_node_attr, use_edge_attr, cleaned)

    def download(self):
        if self.name == 'userDataset':
            print("Using userDataset.")
            return
        super().download()
