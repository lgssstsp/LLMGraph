...
################################################
            ...
            graph = Data(edge_index=edge_index, edge_attr=edge_attr, num_nodes=num_users+num_items)


            transforms_list = []
            for fe_name in args.FEATURE_ENGINEERING:
            ...
            # for fe_name in args.FEATURE_ENGINEERING:
            #     transform_class = getattr(T, fe_name, None)
            #     transform = T.Compose(transform_class)
            #     graph = transform(graph)

                

            users = graph.edge_index[0].numpy()
            items = graph.edge_index[1].numpy()
            ratings = graph.edge_attr.view(-1).numpy()
################################################
...
def load_dataset_userDataset(format, name, dataset_dir):
    if name in ['userDataset']:
        if cfg.dataset.load_type == 'pointwise':
            dataset_file = f'{dataset_dir}/{name}/DS_A.txt'
...
register_loader('userDataset', load_dataset_userDataset)
