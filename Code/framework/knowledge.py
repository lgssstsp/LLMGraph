    ...
    def load_knowledge_base(self, knowledge_type):
        ...
        if knowledge_type == 'prior':
            ...
            for k_type in self.args.knowledge_types:
                ...
                # print(f"[Loaded {len(selected_files)} files into {k_type} category]")
    ...
    def process_pyg_info(self, type='nn'):
        ...
        for entry in pyg_info:
            ...
            try:
                ...
                prompt_general = f"""Please summarize the following paper content. 
                        Focus specifically on the methods discussed in the article, particularly the model design aspects. 
                        Exclude the parts related to experiments, related work, or evaluation. 
                        Here is the text content of the paper:\n
                        """ + pdf_text
                        
                summary = llm_api.call_llm(prompt_general)
                update_entry['Paper Summary'] = summary['answer']
                
                
                
                
                prompt_details = f"""Based on the text content of the paper provided below, please create a structured summary focusing on the design aspects of the model, especially the GNN architecture. The summary should be formatted as a Python dictionary, clearly distinguishing between the model's architectural design and the experimental setup used to validate it.

                Focus on the following key areas and format your response as detailed below:
                - 'GNN_Design': This should include detailed descriptions of the aggregation operators used, skip-connection designs, how information is fused across different layers, number of layers, specific hyperparameters, and activation functions used.
                - 'Experimental_Setup': Despite previous instructions, for a comprehensive understanding, include information about the experiments such as datasets used, a summary of these datasets, baseline models for comparison, and detailed performance comparisons.

                Here are the specific keys for the dictionary:
                {{
                    'GNN_Design': {{
                        'agg_ops': 'Details about aggregation operators',
                        'skip_connections': 'Details about skip-connection designs',
                        'layer_info_fusion': 'How information is fused across layers',
                        'num_layers': 'Total number of layers',
                        'hyperparameters': 'Specific hyperparameters used',
                        'activation': 'Type of activation functions used'
                    }},
                    'Experimental_Setup': {{
                        'datasets': 'List and summary of datasets used',
                        'dataset_summary': 'Detailed descriptions of each dataset',
                        'baseline': 'Baseline models used for comparison',
                        'performance_comparisons': 'Detailed results of performance metrics compared to baselines'
                    }}
                }}

                Return the response in a structured JSON format.
                Each key should precisely correspond to the details requested.
                Here is the text content of the paper:\n""" + pdf_text
                
                summary = llm_api.call_llm(prompt_details)['answer']
                summary = summary.replace('\n', '').replace('```', '').replace('\\', '').replace('json', '').replace('python', '')
                update_entry['Model design and experimental setup'] = summary
