...
parser.add_argument('--data', type=str, default='NCI1', help='dataset name')
...
# parser.add_argument('--hidden_size',  type=int, default=16, help='default hidden_size in supernet')
...
# parser.add_argument('--cos_temp', action='store_true', default=True, help='temp decay')
...
# parser.add_argument('--data_fold', type=int, default=1, help='x_fold cross-validation')
...
genotypes.set_readout_primitives(args.READOUT_PRIMITIVES)
operations.set_readout_ops(args.READOUT_PRIMITIVES)
...
def main(log_filename):
    ...
    data, num_nodes, num_features, num_classes = load_data(args.FEATURE_ENGINEERING, args.data, batch_size=args.batch_size, folds=args.data_fold)
    ...
    for epoch in range(args.epochs):
        ...
        print('[Epoch: {}] Train Loss: {:.4f}, Acc: {:.4f} | Val Loss: {:.4f}, Acc: {:.4f} | Test Loss: {:.4f}, Acc: {:.4f}'.format(
                     epoch, train_loss, train_acc, valid_loss, valid_acc, test_loss, test_acc))
