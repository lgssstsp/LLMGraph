...
def even_quantile_labels(vals, nclasses, verbose=True):
    ...
    if verbose:
        ...
        for class_idx, interval in enumerate(interval_lst):
            print('Class {}: [{}, {})]'.format(class_idx, interval[0], interval[1]))
    return label
...
def load_fixed_splits(dataset, sub_dataset):
    ...
    if not os.path.exists('./data/splits/{}-splits.npy'.format(name)):
        assert dataset in splits_drive_url.keys()
        gdd.download_file_from_google_drive(
            file_id=splits_drive_url[dataset], \
            dest_path='./data/splits/{}-splits.npy'.format(name), showsize=True)
    
    splits_lst = np.load('./data/splits/{}-splits.npy'.format(name), allow_pickle=True)
