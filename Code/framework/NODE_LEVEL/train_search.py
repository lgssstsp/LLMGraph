...
hidden_dict = {'Cora': 128, 'CiteSeer': 128, 'actor':128, 'texas':128, 'squirrel': 64, 'chameleon': 128, 'corafull': 64, 'wisconsin':128, 'cornell':128,
               ...
               'Photo': 32, 
               'Computer': 64, 'arxiv_full': 32,
               'CS': 128, 'physics': 64,
               'deezer-europe': 64,  'yelp-chi': 64, 'arxiv-year': 64, 'others':128, 
               'ogbn-arxiv':64, 'userDataset':64,
               'cstag-computers':128,
               'cstag-photo':128,
               'cstag-history':128,
               'cstag-children':128,
               'cstag-fitness':128,
               'ogbn-proteins':64, 'ogbn-products':32, 'genius':16}
hidden_dict_search_agg = {'Cora': 128, 'CiteSeer': 128,  'actor':128,'texas':128, 'squirrel': 64, 'chameleon':64, 'corafull':32,'wisconsin':128,'cornell':128,
                          ...
                          'Photo': 32, 
                          'Computer': 64, 'arxiv_full': 32,'deezer-europe': 64,
               ...
               'cstag-history':128,
               'cstag-children':128,
               'cstag-fitness':128,'ogbn-proteins':64, 'ogbn-products':32, 'genius':16}
...
hidden_dict_layers = {'Cora': 64, 'CiteSeer': 64,  'actor': 64, 'texas':128, 'squirrel': 64,
                      ...
                      'Photo': 32, 
                      'Computer': 32, 'arxiv_full': 32,'deezer-europe': 64,
                      'CS': 128, 'physics': 32, 'others':64
                      , 'ogbn-arxiv':64, 'userDataset':64,
                                     'cstag-computers':128,
               'cstag-photo':128,
...
parser.add_argument('--data', type=str, default='wisconsin', help='the dataset name')
...
# parser.add_argument('--metric', type=str, default=1, help='')
...
# parser.add_argument('--agg', type=str, default='SAGEConv', help='aggregations used in this framework')
...
parser.add_argument('--temp', type=float, default=0.001, help='temp in softmax')
...
genotypes.set_na_primitives(args.NA_PRIMITIVES)
operations.set_na_ops(args.NA_PRIMITIVES)
genotypes.set_ff_primitives(args.FF_PRIMITIVES)
operations.set_ff_ops(args.FF_PRIMITIVES)


# operations.print_dict()
...
def main(log_filename):
    ...
    feature_engineerings = args.FEATURE_ENGINEERING
    # data, num_features, num_classes = get_dataset(dataset_name, split=split)
    data, num_features, num_classes = get_dataset(dataset_name, feature_engineerings, split=split)
    ...
    if args.data in ['ogbn-proteins', 'genius']:
    ...
    for epoch in range(args.epochs):
        ...
        train_loss, train_acc = train(args.data, data, model, criterion, model_optimizer, arch_optimizer, metric=metric)
        model_scheduler.step()
        t2 = time.time()
        search_cost += (t2 - t1)

        if (epoch + 1) % 10 == 0:
            valid_loss, valid_acc, test_loss, test_acc = infer(args.data, data, model, criterion, metric=metric)
            print(
                ...
                '[Epoch: {}] Train Loss: {:.4f}, Acc: {:.4f} | Val Loss: {:.4f}, Acc: {:.4f} | Test Loss: {:.4f}, Acc: {:.4f}'.format(
                     epoch+1, train_loss, train_acc, valid_loss, valid_acc, test_loss, test_acc))
...
def train(dataset_name, data, model, criterion, model_optimizer, arch_optimizer, metric):
    return train_trans(data, model, criterion, model_optimizer, arch_optimizer, alpha_mode=args.alpha_mode, metric=metric)
def infer(dataset_name, data, model, criterion, single_path=False, metric='acc'):
    val_loss, val_acc = infer_trans(data, model, criterion, test=False, single_path=single_path, metric=metric)
    test_loss, test_acc = infer_trans(data, model, criterion, test=True, single_path=single_path, metric=metric)
    return val_loss, val_acc, test_loss, test_acc

def train_trans(data, model, criterion, model_optimizer, arch_optimizer, alpha_mode='train', metric='acc'):
    model.train()
    total_loss = 0
    accuracy = 0
    ...
    if args.algo in ['random', 'bayes']:
        ...
        logits = model(data)
        # print("data tran:", data)
        # print("Logits shape tran:", logits.shape)
...
def infer_trans(data, model, criterion, test=False, single_path=False, metric='acc'):
    model.eval()
    if args.algo in ['random', 'bayes']:
        logits = model.forward_rb(data, gnn)
    else:
    ...
    if metric=='acc':
        ...
        for i in range(y_true.shape[1]):
            ...
            if np.sum(y_true[:, i] == 1) > 0 and np.sum(y_true[:, i] == 0) > 0:
                ...
                rocauc_list.append(score) 
        acc = sum(rocauc_list)/len(rocauc_list)
...
def run_by_seed():
    res = []
    # print(repr(args)) 
    # print(repr(args.data))  # 使用 repr 打印出原始的字符串内容
    # for key, value in vars(args).items():
    #     print(f"{key}: {repr(value)}")  # 使用 repr 打印每个属性的值，检查是否有非法字符
