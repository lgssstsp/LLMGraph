...
def load_genius():
    ...
    label = F.one_hot(label, label.max() + 1)
...
def load_arxiv_year_dataset(nclass=5):
    filename = 'arxiv-year'
    dataset = NCDataset(filename)
    ogb_dataset = NodePropPredDataset(name='ogbn-arxiv', root=DATAPATH)
    ...
    dataset.label = torch.as_tensor(label)
...
def load_fixed_splits(dataset, sub_dataset):
    ...
    if sub_dataset:
        name += f'-{sub_dataset}'
    if not os.path.exists(DATAPATH + 'splits/{}-splits.npy'.format(name)):
        print()
    ...
    splits_lst = np.load(DATAPATH + 'splits/{}-splits.npy'.format(name), allow_pickle=True)
    print('datasets:{} has {} splits'.format(dataset,len(splits_lst)))
